...
export default function AdminDolibarr() {
  ...
  return (
    <>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Panel>
          <div className="flex items-center gap-2">
            {status.enabled ? <CheckCircle2 size={18} className="text-emerald-400" /> : <XCircle size={18} className="text-slate-500" />}
            <h3 className="font-semibold text-white">Status</h3>
          </div>
          <p className="mt-2 text-sm text-slate-400">{status.enabled ? "Dolibarr aktiv" : "Demo-Modus"}</p>
          <p className="mt-1 text-xs text-slate-500">{conn.message}</p>
          {!status.enabled && (
        ...
        <Panel>
          <div className="flex items-center gap-2"><Database size={18} className="text-brand" /><h3 className="font-semibold text-white">Letzter Sync</h3></div>
          {status.last_sync ? (
            <>
              ...
              <p className="mt-2 text-xs text-slate-500">{new Date(status.last_sync.finished_at).toLocaleString("de-AT")}</p>
            </>
          ) : <p className="mt-2 text-sm text-slate-500">Noch kein Sync.</p>}
        </Panel>
        <Panel>
          <div className="flex items-center gap-2"><Package size={18} className="text-brand" /><h3 className="font-semibold text-white">Produkte im Cache</h3></div>
          <p className="mt-2 text-3xl font-bold text-white">{status.cached_products ?? 0}</p>
        </Panel>
      </div>

      <Panel className="mt-6">
        <h3 className="mb-4 font-semibold text-white">Zwischengespeicherte Produkte</h3>
        {products.length === 0 ? <Empty>Keine Produkte im Cache. Führe einen Sync durch (benötigt aktiven API-Key).</Empty> : (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              <thead className="text-xs uppercase text-slate-500"><tr><th className="py-2">Ref</th><th>Name</th><th>Preis</th><th>Lager</th></tr></thead>
              <tbody className="divide-y divide-subtle">
                {products.map((p) => (
                  <tr key={p.id}><td className="py-2 font-mono text-slate-300">{p.ref}</td><td className="text-white">{p.label}</td><td className="text-brand">{p.price} €</td><td className="text-slate-400">{p.stock ?? "–"}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
      ...
      <Panel className="mt-6">
        <h3 className="mb-4 font-semibold text-white">Sync-Protokoll</h3>
            ...
            {logs.map((l) => (
              <div key={l.id} className="flex items-center justify-between rounded-lg border border-subtle p-3 text-sm">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs text-slate-500">{new Date(l.finished_at).toLocaleString("de-AT")}</span>
                </div>
              </div>
            ))}
          </div>

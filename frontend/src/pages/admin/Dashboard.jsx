  ...
  return (
    <>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {STAT_META.map((s) => (
          <Link key={s.key} to={s.to} className="group rounded-2xl border border-subtle bg-[#0b1220] p-5 transition-colors hover:border-brand/40" data-testid={`stat-${s.key}`}>
            <div className="flex items-center justify-between">
              <span className={`flex h-10 w-10 items-center justify-center rounded-xl ${s.tone === "brand" ? "bg-brand/15 text-brand" : "bg-white/5 text-slate-300"}`}>
                <s.icon size={18} />
              </span>
              <ArrowRight size={16} className="text-slate-600 transition-transform group-hover:translate-x-1 group-hover:text-brand" />
            </div>
            <p className="mt-4 text-3xl font-bold text-white">{data[s.key] ?? 0}</p>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-3">
        <Panel className="lg:col-span-2">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="font-heading font-semibold text-white">Letzte Reparaturanfragen</h2>
              ...
              {data.recent_repairs.map((r) => (
                <div key={r.id} className="flex items-center justify-between gap-3 py-3">
                  <div className="min-w-0">
                    <p className="truncate text-sm font-medium text-white">{r.ref} · {r.device_type} {r.model && `– ${r.model}`}</p>
                    <p className="truncate text-xs text-slate-500">{r.contact?.name} · {r.contact?.email}</p>
                  </div>
        ...
        <Panel>
          <h2 className="mb-4 font-heading font-semibold text-white">Dolibarr</h2>
          <div className="flex items-center gap-2">
            <RefreshCw size={16} className={data.dolibarr_enabled ? "text-emerald-400" : "text-slate-500"} />
            <span className="text-sm text-slate-300">{data.dolibarr_enabled ? "Aktiv" : "Demo-Modus"}</span>
          </div>

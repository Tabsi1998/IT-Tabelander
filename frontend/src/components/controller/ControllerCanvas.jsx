...
// Realistic photo-based controller preview.
// A real transparent PNG of the controller (front/back) is used as the base.
// Selected part colors are applied as blend-mode tint layers that are masked
// to the exact controller silhouette (via the PNG alpha), so recoloring stays
// perfectly on the controller and updates live.
// Falls back to a schematic SVG only if no photo is provided.
...
function TintLayer({ photo, color, opacity = 0.6 }) {
  ...
  return (
    <>
      {/* main hue recolor */}
      <div className="pointer-events-none absolute inset-0" style={{ ...mask, backgroundColor: color, mixBlendMode: "color", opacity: 0.95 }} />
      {/* depth: darken so lighter chosen colors keep the controller's shading */}
      <div className="pointer-events-none absolute inset-0" style={{ ...mask, backgroundColor: color, mixBlendMode: "multiply", opacity }} />
      {/* subtle lift for very dark colors so form stays readable */}
      {!isLight(color) && (
        <div className="pointer-events-none absolute inset-0" style={{ ...mask, backgroundColor: color, mixBlendMode: "screen", opacity: 0.12 }} />
      )}
    </>
  );
}

function PhotoPreview({ photo, side, colors, overlays }) {
  const shell = side === "front" ? colors.shell : colors.back_shell || colors.shell;
  return (
    <div className="relative">
      <img src={photo} alt="Controller" className="relative z-0 w-full select-none" loading="eager" draggable="false" />
      <TintLayer photo={photo} color={shell} opacity={0.55} />
      {/* accent parts that map cleanly onto the whole silhouette are shown via
          overlay PNGs; per-part photographic recolor needs part-level assets */}
        ...
        return (
          <img key={o.id} src={mediaUrl(o.url)} alt="" loading="lazy"
            ...
            style={{
              ...
              transform: `rotate(${l.rotation || 0}deg)`, zIndex: (l.z || 5) + 10,
            }} />
        );
      })}
    </div>
...
// ---- Schematic SVG fallback (kept for controllers without a photo) ----
function SvgFront({ c }) {
  ...
  return (
    <g>
      ...
      <g>
        <circle cx="236" cy="238" r="26" fill={rings} /><circle cx="236" cy="238" r="20" fill="#0c0e11" /><circle cx="236" cy="238" r="15" fill={sticks} />
        <circle cx="322" cy="238" r="26" fill={rings} /><circle cx="322" cy="238" r="20" fill="#0c0e11" /><circle cx="322" cy="238" r="15" fill={sticks} />
      </g>
    </g>
  );
}

function SvgBack({ c, model, hasPaddles, hasBeyond }) {
  ...
  return (
    <g>
      ...
      {[0, 1, 2, 3, 4].map((i) => (<rect key={i} x={250} y={130 + i * 14} width="40" height="5" rx="2" fill="#000" opacity="0.18" />))}
      {hasPaddles && (
      ...
      {hasBeyond && model === "edge" && (<motion.rect initial={{ opacity: 0 }} animate={{ opacity: 1 }} x="248" y="250" width="44" height="30" rx="6" fill={beyond} />)}
    </g>
  );
}
...
export default function ControllerCanvas({ model = "dualsense", side = "front", colors = {}, overlays = [], photoFront = "", photoBack = "", className = "" }) {
  ...
  return (
    <div className={`relative ${className}`}>
      <motion.div key={side + (photo ? "p" : "s")} initial={{ opacity: 0, rotateY: side === "back" ? -14 : 14 }}
        animate={{ opacity: 1, rotateY: 0 }} transition={{ duration: 0.45, ease: [0.16, 1, 0.3, 1] }}>
        {photo ? (
          <PhotoPreview photo={photo} side={side} colors={colors} overlays={overlays} />
        ) : (
          <svg viewBox="0 0 540 400" className="w-full" role="img" aria-label={`${model} ${side}`}>
            ...
            <g filter="url(#cc-shadow)">
              {side === "front" ? <SvgFront c={colors} /> : <SvgBack c={colors} model={model} hasPaddles={hasPaddles} hasBeyond={hasBeyond} />}
